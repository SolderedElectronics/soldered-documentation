---
slug: /soldered-nula-beginner-kit-micropython-hrv/shift-register
title: 7.5 Shift registar
sidebar_label: 7.5 Shift registar
id: shift-register
hide_title: False
---
...
- Kako napisati vlastiti `shift_out()` pomoću operatora **pomaka bita** i **bitovnog I**
...
<InfoBox>Četiri otpornika od **330 Ω** su obični ograničivači struje za LED-ice koje si koristio u svakom primjeru do sada: po jedan za LED-icu, prstenovi **narančasti-narančasti-smeđi**. Upravljačkim pinovima čipa **ne** trebaju otpornici: to su CMOS ulazi i vuku praktički nikakvu struju, pa se spajaju direktno na 3V3 ili na tračnicu mase.</InfoBox>
...
<InfoBox>Čip radi s **3V3**, a ne s `VCC` pina pločice. `VCC` je 5 V koji dolazi preko USB-a, a 74HC595 napojen na 5 V treba 3.5 V da prepozna visoko stanje, više nego što 3.3 V pinovi pločice mogu proizvesti. Radilo bi na stolu a padalo bi negdje drugdje.</InfoBox>
...
1. Povuci zaključavanje **nisko** da se izlazi prestanu mijenjati
2. Pozovi `shift_out()` da pošalje osam bitova, taktirajući svaki od njih unutra
3. Povuci zaključavanje **visoko** da ih prikaže sve odjednom
...
- **`range(7, -1, -1)`** broji **prema dolje**: 7, 6, 5, … 1, 0. Tri broja su početak, kraj i korak, a kraj nikada nije uključen, i zato završava na `-1` da bi dosegnuo 0. Brojanje prema dolje je ono što šalje **najznačajniji bit prvi**, a to je redoslijed koji 74HC595 očekuje.
- **`value >> i`** je **pomak udesno**: pomiče svaki bit broja `i` mjesta udesno, pa bit koji nas zanima završi na dnu.
- **`& 1`** je **bitovni I** s `1`, koji zadržava samo taj donji bit i odbacuje sve iznad njega.
...
<InfoBox>Budući da podaci ulaze na Q0 i pomiču se dalje, prvi poslani bit putuje najdalje i završi na Q7, dok posljednji poslani bit ostaje na Q0. To je ono što Q0 čini jedinicama, Q1 dvojkama, Q2 četvorkama i Q3 osmicama.</InfoBox>
...
<InfoBox>`counter = (counter + 1) % 16` je isti trik obilaženja koji budilica koristi za sate i minute. `%` daje ostatak, pa 16 postaje 0 i brojač počinje ponovno.</InfoBox>
...
<InfoBox>Pritisak na *Stop* ostavlja koji god broj je bio prikazan zamrznut na LED-icama. Zaključavanje ga drži, i ništa ga ne briše do sljedećeg pokretanja ili reseta pločice.</InfoBox>
