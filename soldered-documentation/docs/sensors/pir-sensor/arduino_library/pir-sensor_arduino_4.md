...
This page contains a movement detecting example for the Qwiic version of the board

---

## Initializing the sensor

First, we must include the library and create an instanc of the sensor:
...
Next, in the `setup()` function we are are initializing the sensor at address **0x30** and setting how long the output will be active:
...
Next, in the `loop()` function we are checking if the PIR sensor is available for communication, if it is, we see if there is movement or not and print the state to the serial monitor:
