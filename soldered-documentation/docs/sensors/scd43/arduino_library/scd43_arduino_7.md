...
`sensor.begin()` starts periodic measurement mode automatically. If `begin()` returned `false`, the sensor did not initialize correctly and will never produce readings, so fix the initialization issue first.
