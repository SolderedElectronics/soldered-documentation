...
With the RTC it is possible to set an alarm that sends an interrupt signal when triggered from the INT pin. First, we need to define on what pin we want to listen for an interrupt, for this example we picked GPIO4. We also need to decide what function will be called when an interrupt happens:
...
//Function that will be called when an interrupt happens
...
Next, in the `setup()` function, we have to attach the interrupt to the pin as well as setting what time we want the alarm to happen. For this example, we made it so that an alarm goes off every 59th minute of every hour:
...
void setup()
{
  ...
  //Attach interrupt to the specified pin and have it call the alarm() function when falling edge is detected
...
<InfoBox>If you dont want to set an alarm for a specific parameter, **set its value to 99**, that way it will be ignored</InfoBox>

In the `loop()` function, we are displaying the current time and date very second, just as we did in the previous example:

```cpp
void loop()
{
    //Function that works as the previous example we covered
    printCurrentTime();
    delay(1000);
}
```

When the alarm is triggered, the interrupt happens and we print the message "ALARM TRIGGERED" to the serial monitor:
...
void setup()
{
  ...
  //  setTime(hour, minute, sec);
  rtc.setTime(12, 57, 00); // 24H mode, ex. 6:54:00
  //  setDate(weekday, day, month, yr);
