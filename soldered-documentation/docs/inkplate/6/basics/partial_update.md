...
Instead of calling `display.display()`, you can call `display.partialUpdate()` for a faster display refresh. This method prevents full-screen flickering by updating only the pixels that have changed in the frame buffer.
...
<WarningBox>It is recommended to perform a full update after a certain number of partial updates to maintain both the lifespan and image quality of the e-Paper display. The library performs a full refresh every 10 partial updates by default, and 5 to 10 is the recommended range. Use `display.setFullUpdateThreshold()` to change it.</WarningBox>
...
<FunctionDocumentation
  ...
  returnType="uint32_t"
  parameters={[ 
    { type: 'bool', name: '_forced', description: "Optional. If set to true, a partial update is performed even when the library would otherwise force a full refresh." },
...
<QuickLink 
  title="Inkplate6_Partial_Update.ino" 
  description="Example demonstrating the use of partialUpdate for fast display refreshes on Inkplate 6."
  url="https://github.com/SolderedElectronics/Inkplate-Arduino-library/tree/master/examples/Inkplate6/Basic/Inkplate6_Partial_Update" 
/>