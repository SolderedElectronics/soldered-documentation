...
<SectionTitle title="Buzzer" />

The Inkplate 4TEMPERA features a small **built-in buzzer** that can be used for sound effects, notifications, or simple melodies. It supports tone generation and frequency control through software.

---

## Initialization

Before using the buzzer, it must be initialized with `buzzer.init()`:
...
<FunctionDocumentation
  functionName="inkplate.buzzer.init()"
...
The most straightforward way to produce a sound is by using `buzzer.beep(durationMs)`:

```cpp
inkplate.buzzer.beep(80); // 80ms short beep
```

<FunctionDocumentation
  functionName="inkplate.buzzer.beep()"
...
```cpp
inkplate.buzzer.beep(300, 750);   // 300ms beep at 750Hz
inkplate.buzzer.beep(300, 2400);  // 300ms beep at 2400Hz
```
...
```cpp
inkplate.buzzer.beepOn();   // Turn buzzer on
delay(200);
inkplate.buzzer.beepOff();  // Turn buzzer off
```

<FunctionDocumentation functionName="inkplate.buzzer.beepOn()" description="Turns the buzzer on indefinitely." returnDescription="None" />
<FunctionDocumentation functionName="inkplate.buzzer.beepOff()" description="Turns the buzzer off." returnDescription="None" />
