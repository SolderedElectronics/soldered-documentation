...
def update_sidebar(keyword, category):
    ...
    component_config = f"""        {{
          type: 'category',
          label: '{display_keyword} EDIT THIS NAME',
          items: [
            {{
              type: 'doc',
...
{component_config}
                    ...
                    print(
                        f"Added new Communication category with {keyword} to {sidebar_path}")
                    return True
            else:
...
def create_files_process(keyword, category, doc_dir):
    """Process file creation for a given keyword and category"""
    try:
        # Change to the documentation directory
        os.chdir(doc_dir)
        print(f"Changed directory to: {doc_dir}")
...
"""
        ...
        # Update sidebars.js
        sidebar_result = update_sidebar(keyword, category)
